...
VelodyneFeatureExtraction::VelodyneFeatureExtraction(const YAML::Node& node,
  ...
  if (work_mode == MODE::Odometry_Online) {
    ...
    sub_lidar = nh.subscribe<sensor_msgs::PointCloud2>(
        lidar_topic, 5, &VelodyneFeatureExtraction::LidarHandler, this,
  ...
  pub_feature_cloud =
      nh.advertise<clic::feature_cloud>("lidar_feature/feature_cloud", 10);
...
bool VelodyneFeatureExtraction::ParsePointCloud(
  ...
  if (!has_checked) {
    ...
    check_field_passed = has_ring_field && (has_time_field || has_t_field);

    if (!has_ring_field)
      LOG(WARNING) << "[ParsePointCloud] input cloud NOT has [ring] field";

    if (!has_time_field && !has_t_field)
      LOG(WARNING)
          << "[ParsePointCloud] input cloud NOT has [time] or [t] field";
  }

  /// convert cloud
  if (check_field_passed) {
    if (has_time_field) {
      pcl::fromROSMsg(*lidar_msg, *out_cloud);
    } else if (has_t_field) {
      // for ouster 64
      OusterPointCloud::Ptr ouster_cloud(new OusterPointCloud());
      pcl::fromROSMsg(*lidar_msg, *ouster_cloud);

      OusterPointCloud2RTPointCloud(ouster_cloud, out_cloud);
    }
  }
...
void VelodyneFeatureExtraction::PublishCloud(std::string frame_id) {
  ...
  clic::feature_cloud feature_msg;
