...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time) {
  ...
  // 自适应均衡化图像(若图像太亮或太暗，应在配置文件中将EQUALIZE设为1
  if (EQUALIZE) {
  ...
  if (cur_pts.size() > 0) {
    ...
    if (FLOW_BACK) {
      ...
      cv::calcOpticalFlowPyrLK(forw_img, cur_img, forw_pts, reverse_pts,
                               reverse_status, err, cv::Size(21, 21), 1,
                               criteria, cv::OPTFLOW_USE_INITIAL_FLOW);
      for (size_t i = 0; i < status.size(); i++) {
        if (status[i] && reverse_status[i] &&
            distance(cur_pts[i], reverse_pts[i]) <= FB_THRESHOLD)  // 0.5
          status[i] = 1;
        else
          status[i] = 0;
      }
  ...
  if (PUB_THIS_FRAME) {
    ...
    // 若跟踪角点数小于 MAX_CNT，则选取一些特征以弥补跟踪角点数量逐渐减少
    TicToc t_t;
...
void FeatureTracker::addPoints(std::vector<cv::Point2f> &n_pts) {
  for (auto &p : n_pts) {
...
void FeatureTracker::reducePoints(vector<uchar> &status) {
