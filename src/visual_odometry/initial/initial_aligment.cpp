...
namespace clic {
void VisualIMUAlignment::solveGyroscopeBias(
    map<double, ImageFrame> &all_image_frame, Vector3d *Bgs) {
  Matrix3d A;
  Vector3d b;
  Vector3d delta_bg;
  A.setZero();
...
MatrixXd VisualIMUAlignment::TangentBasis(Vector3d &g0) {
  Vector3d b, c;
...
void VisualIMUAlignment::RefineGravity(map<double, ImageFrame> &all_image_frame,
                                       Vector3d &g, VectorXd &x) {
  Vector3d g0 = g.normalized() * std::fabs(GRAVITY_NORM);
  Vector3d lx, ly;
  // VectorXd x;
  ...
  for (int k = 0; k < 4; k++) {
    ...
    for (frame_i = all_image_frame.begin();
      ...
      tmp_b.block<3, 1>(0, 0) =
          ...
          frame_i->second.R.transpose() * frame_j->second.R * p_CtoI - p_CtoI -
    ...
    g0 = (g0 + lxly * dg).normalized() * std::fabs(GRAVITY_NORM);
    // double s = x(n_state - 1);
  }
  g = g0;
}

bool VisualIMUAlignment::LinearAlignment(
    map<double, ImageFrame> &all_image_frame, Vector3d &g, VectorXd &x) {
  int all_frame_count = all_image_frame.size();
  ...
  if (fabs(g.norm() - std::fabs(GRAVITY_NORM)) > 1.0 || s < 0) {
    return false;
  }
...
bool VisualIMUAlignment::TryAlign(map<double, ImageFrame> &all_image_frame,
