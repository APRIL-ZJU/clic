...
bool MotionEstimator::solveRelativeRT(
  ...
  if (corres.size() >= 15) {
    ...
    // [mask.rows, mask.cols] = [len, 1]
    if (inliners) {
      inliners->reserve(inlier_cnt);
      for (int i = 0; i < mask.rows; i++) {
        if (mask.at<char>(i) > 0) inliners->push_back(i);
      }
      // std::cout << inliners->size() << "; inlier_cnt: " << inlier_cnt <<
      // "\n";
    }
    ...
    if (inlier_cnt > 12)
      ...
      // std::cout << "[solveRelativeRT] inlier/corres: " << inlier_cnt << "/"
