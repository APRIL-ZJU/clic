...
IMUInitializer::IMUInitializer(const YAML::Node &node) : initial_done_(false) {
  window_length_ = yaml::GetValue<double>(node, "window_length", 1.0);
  ...
  std::cout << "gravity_mag1: " << gravity_mag << std::endl;
...
// https://github.com/rpng/open_vins/blob/master/ov_init/src/static/StaticInitializer.cpp
bool IMUInitializer::StaticInitialIMUState() {
  ...
  if (accel_var >= imu_excite_threshold_) {
    LOG(WARNING) << "[InertialInitializer] Dont Move !";
    return false;
  }
